...
static inline size_t dmlite_sizeof_handle(struct file_handle *hdl)
{
	return offsetof(struct file_handle, f_handle) + hdl->handle_bytes;
}

#define DMLITE_HANDLE_LEN 48
typedef struct dmlite_file_handle {
        unsigned int handle_bytes;
        int handle_type;
        unsigned char handle[DMLITE_HANDLE_LEN];
} dmlite_file_handle_t ;


static inline int dmlite_name_to_handle(const char *name, dmlite_file_handle_t *fh, int *mnt_id)
{
  return name_to_handle_at(AT_FDCWD, name, (struct file_handle *)fh, mnt_id, AT_SYMLINK_FOLLOW);
}

static inline int dmlite_lname_to_handle(const char *name, dmlite_file_handle_t *fh, int *mnt_id )
{
  return name_to_handle_at(AT_FDCWD, name, (struct file_handle *)fh, mnt_id, 0);
}

static inline int dmlite_fd_to_handle(int fd, dmlite_file_handle_t * fh, int *mnt_id)
{
  return name_to_handle_at(fd, "", (struct file_handle *)fh, mnt_id, AT_EMPTY_PATH);
}

static inline int dmlite_open_by_handle(int mountfd, dmlite_file_handle_t * fh, int flags)
{
  return open_by_handle_at(mountfd, (struct file_handle *)fh, flags);
}

static inline int dmlite_name_by_handle_at(int atfd, const char *name, dmlite_file_handle_t *fh)
{
  int mnt_id;
...
static inline ssize_t dmlite_readlink_by_handle(int mountfd, dmlite_file_handle_t *fh, char *buf, size_t bufsize)
{
        int fd, ret;

        fd = dmlite_open_by_handle(mountfd, fh, (O_PATH|O_NOACCESS));
        if (fd < 0)
                return fd;
...
static inline int dmlite_stat_by_handle(int mountfd, dmlite_file_handle_t *fh, struct stat *buf)
{
        int fd, ret;
        fd = dmlite_open_by_handle(mountfd, fh, (O_PATH|O_NOACCESS));
        if (fd < 0)
...
static inline int dmlite_link_by_handle(int mountfd, dmlite_file_handle_t *fh, int newdirfd, char *newname)
{
        int fd, ret;
        fd = dmlite_open_by_handle(mountfd, fh, (O_PATH|O_NOACCESS));
        if (fd < 0)
...
static inline int dmlite_chown_by_handle(int mountfd, dmlite_file_handle_t *fh, uid_t owner, gid_t group)
{
        int fd, ret;
        fd = dmlite_open_by_handle(mountfd, fh, (O_PATH|O_NOACCESS));
        if (fd < 0)
